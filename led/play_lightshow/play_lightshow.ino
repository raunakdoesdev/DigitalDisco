...
void light_show_flash(double* timestamp) {
  ...
  switch (state) {
    ...
    case COLOR:
      ...
      for (int dot = 0; dot < 10; dot++) { // not dot < NUM_LEDS bc that seems to cause power surge
        leds[dot] = CHSV(freq, 250, 127);
      }
      state = RETAIN;
      break;
    case RETAIN:
...
void light_show_chase(double* timestamp) {
  ...
  switch (state) {
    ...
    case COLOR:
      freq = frequencies[notes - 1];  // len(freq) = len(beats-1)
      leds[dot] = CHSV(freq, 250, 127); // change single dot (starts at 0)
      leds[dot + 1] = CHSV(freq, 250, 127);
      state = RETAIN;
      break;
    case RETAIN:
      while (millis() - start < timestamp[notes]) {
